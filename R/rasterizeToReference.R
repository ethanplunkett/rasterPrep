...
#' @param destination (character) path to a `.tif` file that will either be created or added to.
...
#' @param burn (optional, numeric) value to be burnt into grid (everywhere the `source` has data)
...
#' @param type the type of grid to create. Should be one of `"Byte"`, `"UInt16"`,
#' `"Int16"`, `"UInt32"`, `"Int32"`, `"Float32"`,
#' `"Float64"` or for convenience you may
...
#'   see "-at" flag description for gdal_rasterize: <http://www.gdal.org/gdal_rasterize.html>
...
rasterizeToReference <- function(source, destination, reference, burn, attribute,  init,
  ...
  if(!dest.exists){
    ...
    intermediatefile <- gsub(".tif$", "_tempzzz.tif", destination,
                             ignore.case = TRUE)
  }
