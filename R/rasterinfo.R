...
#'  \item{driver}{The driver used to read the file which usually matches
#'  the file's format}
#'  \item{type}{Calculated from `bands` will be "missing" if the raster file is
#'  missing or unreadable, "mixed" if there are multiple bands and they have
#'  varying types, otherwise the type associated with the band(s).}
#'  \item{res}{the resolution of  the cells,  as reported gdal (two dimensions,
#'  possible negative)}
#'  \item{cellsize}{calculated from `res` `NA` if the cells aren't square,
#'  otherwise the (positive) dimension of the cell}
#'  \item{na.value}{`NA` if bands have inconsistent no data values. Otherwise
#'  the no data value used by the band(s)}
#'  \item{dim}{The dimensions of the raster in cells (rows, then cols).  This
#'  is from a raw line that reads  "Size is" and reports cols then rows
#'  (reverse of order used here). }
#'  \item{rows}{the number of rows in the file  (copied from `dim`)}
#'  \item{cols}{the number of columns in the file (copied from `dim`)}
#'  \item{origin}{the coordinates of the origin of the file
#'  (this seems to be the upper left corner).
#'  Note, gdal may report more digits than R can represent.
#'  Look at raw output to see the text as reported by gdal}
#'  \item{approx.coords}{the approximate coordinates of the corners and center.
#'  This is reported by `gdalinfo` but appears to be rounded, thus the "approx"}
#'  \item{extent}{A list with items `xll`, `yll`, `nrow`, `ncol`, `cellsize`.
#'  If cells are square these are calculated from  `origin`, `rows`,
#'  `cols` and `cellsize`.
#'  This format matches the way grid extents are defined in \pkg{gridio2}.}
#'  \item{nbands}{The number of bands in the file.}
#'  \item{bands}{A list  of lists (one per band); inner list has items
#'  \describe{
...
#'  \item{attributetable}{the raster attribute table
#'  (AKA Value Attribute Table, VAT)}
#'  \item{raw}{The raw text returned by gdalinfo that was parsed to create all
...
rasterInfo <- function(x) {
  ...
  #----------------------------------------------------------------------------#
  # Separate out the Raster Attribute Table
  # (mostly because it's long and annoying)
  #----------------------------------------------------------------------------#
  ...
  if (length(end) > 1 || length(start) > 1)
    stop("More than one GDALRasterAttributeTable. This is unexpected.")
  if (length(start) != length(end))
    stop("There should be either 1 start and 1 end of the attribute table",
         "or no start and end")

  if (length(start) == 1) {
    result$attributetable <- l[start:end]  # move to result
    l <- l[setdiff(seq_along(l), start:end)] # cut out from l
  }
  rm(start, end)
  #----------------------------------------------------------------------------#
  #----------------------------------------------------------------------------#
  ...
  #----------------------------------------------------------------------------#
  # Files:  (start at Files: end at first subsequent line that doesn't start
  # with blank space)
  #----------------------------------------------------------------------------#
  ...
  # drop files tag from first files line
  files[1] <- gsub("^Files:[[:blank:]]*", "", files[1], ignore.case = TRUE)
  files <- trimws(files)
  ...
  #     Allow one line without leading blank and then stop at first line
  #     without a leading blank
  #----------------------------------------------------------------------------#
  start <- grep("^Coordinate System is:", l, ignore.case = TRUE)
  if (length(start) > 1)
    stop("More than one Coordinate System is: line\n")
  if (length(start) == 1) {
    end <- start + 1
    while (TRUE) {
      if (!grepl("^[[:blank:]]", l[end + 1]))
  ...
  if (length(size) != 2)
    stop("Expected only two values for Size (cols, rows) found", length(size))
  if (any(grepl("[^[:digit:].]", size)))
    stop("There appears to be non numeric values after Size is. I'm finding:",
         paste(size, collapse = ", "))
  ...
  #----------------------------------------------------------------------------#
  # Pixel Size: assigned to "res" and if both are equal also to cellsize
  #----------------------------------------------------------------------------#
  ln <- grep("^Pixel size =", l, ignore.case = TRUE)
  if (length(ln) != 1) {
    # Previously I threw an error here as of Nov 2022 I'm assuming if there's
    # no pixel size line that the target isn't
    # a raster GIS file. And returning a type = "missing" as per documentation.
    # This is known to happen if you call rasterInfo on a standard (non geo)
    # TIFF
    result$type <- "missing"
    return(result)
  }
  ...
  if (length(res) != 2)
    stop("Expected only two values for pixel size (xdim, ydim) found",
         length(res))
  ...
  if (length(start) > 1)
    stop("Found more than one non-indented Metadata tag. ",
  ...
  if (length(start) == 0) {
    ...
    # extract whitespace from beginning first line after start:
    indent <- gsub("(^[[:blank:]]*)(.*$)", "\\1", l[start], perl = TRUE)
    while (TRUE) {
      if (!grepl(paste0("^", indent), l[end + 1]))
        break
      end <- end + 1
      if (end == length(l))
        break
  ...
  if (length(start) > 1)
    stop("Found more than one non-indented Metadata tag. Expected at most one.")
  if (length(start) == 0) {
  ...
  while (any(grepl("^Band ", l))) {
    ...
    if (length(ln) != 1)
      stop("Expected one and only one Band= for this band")
    ...
    if (!has.full.stats) {
      #There seems to be an alternate min and max value on a line without other
      # statistics.  I'm not sure if always present but if there aren't
      # full stats and this line exists I'm going to grab it
      ln <- grep("Min=.*Max=", b)
    ...
    if (length(start) == 0) {
      ...
      # extract whitespace from beginning first line after start:
      indent <- gsub("(^[[:blank:]]*)(.*$)", "\\1", b[end], perl = TRUE)
      while (TRUE) {
        if (!grepl(paste0("^", indent), b[end + 1]))
          break
        end <- end + 1
        if (end == length(b))
          break
    ...
    # Note this will be a Image Structure Metadata: line  with leading
    # white space
  ...
  extract.coords <- function(tag, x) {
    ...
    if (length(a) > 1)
      stop("Duplicated tags")
    x <- x[a]
    # drop everything up until first "(" and everything  after first ")":
    x <- gsub("^[^(]*\\(|\\).*$", "", x = x)
    x <- unlist(strsplit(x, ","))
    x <- gsub("[[:blank:]]", "", x)
    x <- as.numeric(x)
  ...
  if (!all(abs(origin - approx.coords[1, ])  < abs(result$res) / 100))
    stop("I'm assuming that the origin is the upper left ",
         "but that doesn't appear to be true.")

  result$origin <- origin
  result$approx.coords <- approx.coords

  # add convenience tags nbands and type, derved from info in bands
  ...
  if (!is.na(result$cellsize)) {
    ...
    if (!all(abs(c(xll, yll) - approx.coords[2, ])  < abs(result$res) / 100))
      stop("xll and yll don't correspond to lower left corner. ",
           "This file isn't structured the way  I assumed it was.")
    result$extent <- list(xll = xll,
                          yll = yll,
                          nrow = result$rows,
                          ncol = result$cols,
  ...
  if (!is.list(x)) {
    if (is.vector(x)) {
      if (is.numeric(x)) {
        ...
        spacer <- paste0("\n",
                         paste(rep("  ", indent - 1), collapse = "", sep = ""),
                         paste(rep(" ", nchar(name)), collapse = "", sep = ""),
                         "   ")
      }
...
print.rasterInfo <- function(x, all = FALSE, raw = FALSE, ...) {
  ...
  printnestedlist(x)
  if (!all)
    cat("[Some items may have been skipped use print(x, all=TRUE) ",
        "to print everything.]", sep = "")
}
