.extract.tag <- function(tag, x, numeric = FALSE){
  # This helper function extracts the non blank text after the tag continuing to next blank.
  # It throws an error if the tag occurs more than once
  # If numeric is TRUE converts it to a number
  ln <- grep(tag, x, ignore.case = TRUE)
  if(length(ln) > 1) 
    stop("More than one", tag, "tag found." )
  if(length(ln) == 0) 
    return(NA)
  res <- gsub(paste0("(^.*", tag,"[[:blank:]]*)([^[:blank:]]*)(.*$)"), "\\2", x[ln], ignore.case = TRUE)
  ...
  if(numeric){
    ...
    if(is.character(res) && grepl("[^[:digit:]\\.+e-]", res, ignore.case = TRUE))
